...
type InvoiceClientsProps = StackScreenProps<
  RootStackParamList,
  "InvoiceClients"
>;
  ...
  const handleDelete = () => {
    ...
    if (selectedItem.id) {
      ApiService.postWithToken(
        `api/invoice-generator/customer/delete/${selectedItem.id}`,
        {}
      ).then((res) => {
  ...
  const renderItem = ({ item }: any) => (
    <View style={[styles.clientCard, { backgroundColor: colors.card }]}>
      ...
      >
        ...
        <View>
          <Text style={[styles.clientName, { color: colors.title }]}>
            {item.name}
          </Text>
          <Text style={[styles.clientEmail, { color: colors.title }]}>
            {item.email}
          </Text>
        </View>
      </TouchableOpacity>
    ...
    <>
      ...
      >
        ...
        {customer.length > 0 ? (
          ...
          <Text style={{ textAlign: "center", fontSize: 12, marginTop: 40 }}>
            No Client Found
          </Text>
        )}
        {/* {isLoading ? (
