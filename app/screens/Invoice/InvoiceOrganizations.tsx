...
type InvoiceOrganizationsProps = StackScreenProps<
  RootStackParamList,
  "InvoiceOrganizations"
>;

export const InvoiceOrganizations = ({
  navigation,
}: InvoiceOrganizationsProps) => {
  ...
  const fetchCompanyList = () => {
    setIsLoading(true);
    ApiService.postWithToken("api/invoice-generator/companies/list", {}).then(
      (res) => {
        if (res.status) {
          setCompanyData(res);
        }
  ...
  const handleDelete = () => {
    ...
    if (selectedItem.id) {
      ApiService.postWithToken(
        `api/invoice-generator/companies/delete/${selectedItem.id}`,
        {}
      ).then((res) => {
    ...
    <View style={[styles.organizationCard, { backgroundColor: colors.card }]}>
      ...
      >
        ...
        <View style={styles.textContainer}>
          <Text style={[styles.organizationName, { color: colors.title }]}>
            {item.name}
          </Text>
      ...
      <View style={styles.container}>
        ...
        <View
          style={[
            ...
            { backgroundColor: colors.background, elevation: 4 },
          ]}
        >
          <MaterialIcons
            name="search"
        ...
        >
          <View style={styles.bottomSheetContent}>
            ...
            <TouchableOpacity
              onPress={() => {
                ...
                navigation.navigate("InvoiceAddOrganization", {
                  orgranisationData: {
                    ...selectedItem,
                    imagepath: companyData.image_path,
                  },
                });
              }}
            ...
            >
              ...
              <Text
                style={[styles.sheetButtonText, { color: COLORS.background }]}
              >
                Edit
              </Text>
            </TouchableOpacity>
